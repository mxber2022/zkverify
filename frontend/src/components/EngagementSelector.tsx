...
export function EngagementSelector({ selected, onSelect }: EngagementSelectorProps) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {options.map(({ id, icon: Icon, label }) => (
        ...
        >
          <Icon className={`w-4 h-4 ${selected === id ? 'text-gray-800' : 'text-gray-600'}`} />
          <span className={`text-sm font-medium ${selected === id ? 'text-gray-800' : 'text-gray-600'}`}>
            {label}
          </span>
        </button>
      ))}
